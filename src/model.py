...
class TrainModule(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        ...
        for i, name in enumerate(self.class_names):
            self.custom_logger.report_scalar(f'rocauc_{name}', 'train', float(train_metrics['train_auroc'][i]), self.current_epoch)

            self.custom_logger.report_scalar(f'rocauc_{name}', 'train', float(train_metrics['train_f1'][i]), self.current_epoch)
    ...
    def validation_epoch_end(self, outputs):
        ...
        for i, name in enumerate(self.class_names):
            self.custom_logger.report_scalar(f'rocauc_{name}', 'val', float(val_metrics['val_auroc'][i]), self.current_epoch)

            self.custom_logger.report_scalar(f'rocauc_{name}', 'val', float(val_metrics['val_f1'][i]), self.current_epoch)
